        ...
        setSelectDate: function( date ) {
            ...
            if ( curSltItem.length ) {
                ...
                if ( !curSltItem.hasClass( 'jr' ) ) {
                    ...
                    curDateNameEl.text( config.selectDateName[0] );
                }
            }
        },
        select: function(date,status) {
             var me = this,
                config = this.config,
                date = ( typeof date == 'string' ) ? date : util.formatDate( date ),
                dateNum = util.dateToNum( date ),
            ...
            if ( curSltItem.length ) {
                var curDateNameEl = $( curSltItem.find( 'i' )[ 1 ] );
                if ( status == 0 ){
                     curSltItem.addClass( 'ing' );
                    curDateNameEl.text( config.selectDateName[1] );
                }
                else if( status == 1 ){
                    curSltItem.addClass( 'done' );
                     curDateNameEl.text( config.selectDateName[2] );
                }
                else if ( status == 2 ) {
                    curSltItem.addClass( 'un' );
                     curDateNameEl.text( config.selectDateName[3] );
                }
                else{
                    curSltItem.addClass( 'cur' );
                     curDateNameEl.text( config.selectDateName[0] );
